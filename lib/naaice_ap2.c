...
// Specification of the NAA Functions to be used (see parse_naa_job_sec for details)
...
static void parse_naa_job_spec(void)
{
	...
	if (naa_spec_env == NULL) {
		log_error("Environment variable " NAA_JOB_SPEC_ENV_VAR_NAME " not set! No NAAs useable");
		return;
	}

	char *spec = strdup(naa_spec_env);
	if (spec == NULL) {
		log_warn("Unable to get NAA job spec provided via " NAA_JOB_SPEC_ENV_VAR_NAME " environment variable. No NAAs useable");
		return;
	}
	...
	if (!naa_job_spec) {
		free(spec);
		log_error("Memory allocation for job specification failed");
		return;
	}

	char *tokenized_spec = spec, *chunk;
	num_naa_job_spec = 0; // reset to zero to count the number of correctly parsed job specifications
	...
	if (num_naa_job_spec == 0) {
		log_error("No valid NAA job specs provided. No NAAs will be useable");
	}
}
...
static naa_job_spec_item_t* get_naa_for_function_code(const naa_function_code_t fn_code)
{
	...
	for (size_t i = 0; i < num_naa_job_spec; i++) {
		if (naa_job_spec[i].handle == NULL && naa_job_spec[i].fn_code == fn_code) {
			log_info("using remote address %s (fn_code %d), port %d", naa_job_spec[i].address, fn_code, naa_job_spec[i].port);
	...
	naa_handle *handle)
{
	...
	if (naa == NULL) {
		log_error("No NAA for function code %d available.", function_code);
		return -1;
	}

	if (naa->param_count != input_amount) {
		log_error("Mismatch between number of arguments for NAA at %s (fn_code %d). %d configured, %d provided by application",
    ...
    // FIXME: Memory Leak (?). Why not declare as uint32_t and naaice_set_immediate masks as needed
	...
	for (unsigned int i = 0; i < input_amount; i++) {
		...
		if (!param_exists) {
			log_error( "Requested input parameter which was not previously passed to naaice_init_communication_context.\n");
			return -1;
		}
	}
	...
	for (unsigned int i = 0; i < output_amount; i++) {
		...
		if (!param_exists) {
			log_error( "Requested output parameter which was not previously passed to naaice_init_communication_context.\n");
			return -1;
		}
	}
