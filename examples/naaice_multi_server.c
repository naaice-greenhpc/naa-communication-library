...
int main(int argc, __attribute__((unused)) char *argv[]) {
  ...
  size_t current_connection = 0;
  struct context *ctx;
  pthread_t worker_thread[MAX_CONNECTIONS];
  ...
  struct thread_args thread_args_array[MAX_CONNECTIONS];

  naaice_swnaa_init_master(&ctx, CONNECTION_PORT);
  while (1) {

    ctx->master->state = NAAICE_INIT;
    if (naaice_swnaa_setup_connection_multi(ctx)) {
